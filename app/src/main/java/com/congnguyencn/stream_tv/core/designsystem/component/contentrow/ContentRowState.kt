...
@Stable
class ContentRowState internal constructor(initialSelectedIndex: Int) {
  ...
  internal fun updateLayoutInfo(bounds: ContentRowSelectionBounds, forwardStepPx: Float, backwardStepPx: Float) {
  ...
  private fun canMoveSelection(direction: Int): Boolean {
    if (direction == 0 || itemCount <= 1 || isScrollInProgress) return false
  ...
  internal suspend fun moveSelection(
    ...
    if (!canMoveSelection(direction)) return
...
internal data class ContentRowSelectionBounds(val left: Int, val top: Int, val width: Int, val height: Int) {
  companion object {
