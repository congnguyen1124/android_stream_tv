...
- Online images are loaded with Coil 3, while the dummy catalogue supplies stable content and artwork metadata for local development.
...
└── presentation/
    ├── component/              # HomeContent, Banner, VerticalBanner, ContentRow section, cards
    ├── mapper/                 # Domain -> UI model
    ├── model/                  # UI items and UI view types
    ├── HomeRoute.kt            # HomeScreen binds the ViewModel
    ├── HomeUiState.kt
    └── HomeViewModel.kt
```
...
`MainScreen` is the content-browsing shell: it owns `StreamTvTopBar` and a nested `MainNavHost` for
...
`HomeViewModel` calls the suspend repository directly inside `viewModelScope`, cancels the previous
request on reload, and does not swallow `CancellationException`.
...
Every content item has a stable `id` plus the descriptive and artwork metadata appropriate to its
type. The `id` gives Compose a stable key.
