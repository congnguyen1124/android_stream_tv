...
class ContentRowState internal constructor(
    ...
    /** Immediately selects [index], wrapping it into the current backing collection. */
    suspend fun scrollToItem(index: Int) {
        if (itemCount == 0) return
        Snapshot.withMutableSnapshot {
            selectedIndex = index.floorMod(itemCount)
    ...
    internal fun updateItemCount(newItemCount: Int) {
        ...
        Snapshot.withMutableSnapshot {
            ...
            selectedIndex = if (newItemCount == 0) 0 else selectedIndex.floorMod(newItemCount)
            if (newItemCount == 0) {
                animationOffsetPx = 0f
...
internal fun contentRowTargetIndex(currentIndex: Int, delta: Int, itemCount: Int): Int {
    ...
    return when {
        delta > 0 && currentIndex >= itemCount - 1 -> 0
